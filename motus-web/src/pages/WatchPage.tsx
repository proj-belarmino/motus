...
export default function WatchPage() {
  ...
  const applySubtitleSelection = useCallback(
    (video: HTMLVideoElement, subtitleId: string | null) => {
      const tracks = video.textTracks;
      for (let i = 0; i < tracks.length; i++) {
        const track = tracks[i];
        const target = subtitleId
          ? subtitleTrackRefs.current.get(subtitleId)
          : undefined;
  ...
  return (
    ...
    >
      ...
      <div
        className={`pointer-events-none absolute inset-0 bg-gradient-to-b from-black/70 via-transparent to-black/85 transition-opacity duration-300 ${showControls ? "opacity-100" : "opacity-0"}`}
      />

      <header
        className={`absolute inset-x-0 top-0 z-10 flex items-center justify-between p-4 sm:p-6 transition-all duration-300 ${showControls ? "translate-y-0 opacity-100" : "-translate-y-4 opacity-0 pointer-events-none"}`}
      >
        <button
          onClick={() => navigate(-1)}
      ...
      {showSettings && (
        <section
          className="absolute right-4 top-20 z-20 w-72 rounded-2xl border border-white/15 bg-zinc-950/95 p-3 text-sm shadow-2xl backdrop-blur-xl sm:right-6 sm:top-24"
        ...
        >
          ...
          <label className="flex cursor-pointer items-center justify-between px-2 py-4">
            ...
            <input
              type="checkbox"
              checked={transcode}
              onChange={(event) => setTranscode(event.target.checked)}
              className="h-4 w-4 accent-primary"
            />
          </label>
          <div className="border-t border-white/10 px-2 pt-3">
            <span className="mb-2 flex items-center gap-2 text-white/70">
            ...
            <div className="grid grid-cols-4 gap-1">
              {[0.75, 1, 1.25, 1.5].map((rate) => (
                <button
                  ...
                  className={`rounded-lg px-2 py-2 text-xs font-medium transition ${playbackRate === rate ? "bg-primary text-white" : "bg-white/8 text-white/70 hover:bg-white/15"}`}
                >
                  {rate}×
                </button>
              ))}
            ...
            {movie?.subtitles?.length ? (
              <div className="space-y-1">
                <button
                  ...
                  className={`w-full rounded-lg px-2 py-2 text-left text-xs font-medium transition ${activeSubtitleId === null ? "bg-primary text-white" : "bg-white/8 text-white/70 hover:bg-white/15"}`}
                >
                  Off
                </button>
                {movie.subtitles.map((subtitle) => (
                  <button
                    ...
                    className={`flex w-full items-center justify-between gap-2 rounded-lg px-2 py-2 text-left text-xs font-medium transition ${activeSubtitleId === subtitle.id ? "bg-primary text-white" : "bg-white/8 text-white/70 hover:bg-white/15"}`}
                  >
                    <span className="truncate">{subtitle.label}</span>
                    <span className="shrink-0 rounded bg-white/10 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider">
                      {subtitle.language}
                    </span>
                  </button>
                ))}
              ...
              <p className="text-xs text-white/45">
                No subtitles attached to this title.
              </p>
            )}
          </div>
        </section>
      ...
      <section
        className={`absolute inset-x-0 bottom-0 z-10 p-4 sm:p-6 transition-all duration-300 ${showControls ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0 pointer-events-none"}`}
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              <div className="ml-1 flex items-center gap-2">
                ...
                <input
                  ...
                  className="hidden h-1 w-20 accent-primary sm:block"
                  aria-label="Volume"
                />
              </div>
              <span className="ml-1 whitespace-nowrap text-xs font-medium tabular-nums text-white/85 sm:text-sm">
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              <button
                onClick={() => setShowSettings((open) => !open)}
                className="hidden items-center gap-1 rounded-md px-2 py-2 text-xs font-medium text-white/85 transition hover:bg-white/15 sm:flex"
                aria-label="Change playback speed"
              >
                {playbackRate}× <ChevronDown className="h-3 w-3" />
              </button>
              <ControlButton
...
function ControlButton({
  ...
  return (
    <button
      onClick={onClick}
      className="flex h-9 w-9 items-center justify-center rounded-full text-white transition hover:bg-white/15 focus:outline-none focus:ring-2 focus:ring-white"
      aria-label={label}
    >
      {children}
    </button>
