...
import { Image as ImageIcon, LoaderCircle, Plus, Star, Tv, X } from "lucide-react";
...
export default function CreateShowModal({ onClose, onCreated }: CreateShowModalProps) {
  const api = useApi();
  ...
  const handleCreate = useCallback(async (title: string, tmdbId?: number) => {
    setCreating(true);
    setError("");
    try {
  ...
  return (
    <div className="fixed inset-0 z-[70]" role="dialog" aria-modal="true" aria-label="Create a show">
        ...
        <div className="animate-rise-in overflow-hidden rounded-2xl border border-border bg-surface shadow-2xl">
          <div className="flex items-center gap-3 border-b border-border px-4 py-4 sm:px-5">
            ...
            {loading ? (
              ...
              <button onClick={onClose} className="rounded-lg p-1.5 text-muted transition hover:bg-surface-hover" aria-label="Close">
                <X className="h-4 w-4" />
              </button>
            )}
          </div>
          ...
          <div className="max-h-[min(60vh,480px)] overflow-y-auto p-2">
            {showEmpty ? (
              <div className="flex flex-col items-center gap-3 px-4 py-12 text-center">
                ...
                <div>
                  <p className="font-semibold text-foreground">No matches for “{trimmedName}”</p>
                  <p className="mt-1 text-sm text-muted">You can still create the show manually below.</p>
                </div>
              </div>
            ) : loading ? (
              <div className="space-y-1">
                {Array.from({ length: 6 }, (_, index) => (
                  <div key={index} className="animate-soft-pulse flex items-center gap-3 rounded-xl p-2">
              ...
              results.map((result) => {
                ...
                return (
                  <button
                    key={result.tmdbId}
                    onClick={() => void handleCreate(trimmedName, result.tmdbId)}
                    disabled={creating}
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="mt-0.5 block text-xs text-muted">
                        {result.year ? `${result.year} · ` : ""}{result.genres.slice(0, 2).join(", ") || "TV"}
                      </span>
                      {result.overview ? <span className="mt-1 line-clamp-1 block text-xs text-muted/80">{result.overview}</span> : null}
                    </span>
                    {result.rating ? (
                      <span className="flex shrink-0 items-center gap-1 text-sm font-medium text-amber-300">
                        <Star className="h-3.5 w-3.5 fill-current" /> {result.rating}
                      </span>
                    ) : null}
                  </button>
                );
          ...
          {error && <p className="border-t border-border px-5 py-3 text-sm font-medium text-red-500">{error}</p>}

          <div className="flex items-center justify-between gap-3 border-t border-border px-4 py-3 sm:px-5">
            <span className="text-xs text-muted">Pick a match or create it manually with any name.</span>
            <button
