    ...
    >
      <div className="animate-rise-in relative flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-border bg-background shadow-2xl">
        ...
        <div className="relative h-60 w-full shrink-0 bg-black sm:h-[40vh]">
          ...
          <div className="absolute bottom-5 left-5 flex items-center space-x-4 sm:bottom-6 sm:left-8">
            ...
            >
              {savingBookmark === "watchlist" ? (
                <LoaderCircle className="h-5 w-5 animate-spin" />
              ) : (
                <Bookmark
                  className={`h-5 w-5 ${isWatchlisted ? "fill-current" : ""}`}
                />
              )}
            </button>
          </div>
        ...
        <div className="overflow-y-auto px-5 py-6 text-foreground sm:px-8">
          {feedback && (
            <div className="animate-rise-in mb-4 flex items-center justify-between gap-3 rounded-xl border border-emerald-500/25 bg-emerald-500/10 px-4 py-3 text-sm font-medium text-emerald-700 dark:text-emerald-300">
            ...
            {!isRenaming && (
              <div className="flex gap-2">
                <button
                  ...
                  className="flex items-center space-x-2 rounded-xl border border-border bg-surface px-3 py-2 text-sm font-medium text-foreground transition hover:bg-surface-hover disabled:opacity-50"
                  title="Regenerate thumbnail and fetch metadata"
                >
                  <RefreshCw
                ...
                <button
                  ...
                  className="flex items-center gap-2 rounded-xl border border-red-500/25 px-3 py-2 text-sm font-medium text-red-500 transition hover:bg-red-500/10"
                  title="Delete movie"
                >
              ...
              <div className="rounded-lg border border-border bg-surface p-4 space-y-3">
                <div className="mb-1 flex items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    className="flex items-center space-x-2 rounded-xl border border-border bg-background px-3 py-1.5 text-xs font-semibold text-foreground transition hover:bg-surface-hover disabled:opacity-50"
                    title="Upload subtitles"
                  >
                    <Upload className="h-3.5 w-3.5" />
                    <span>
                      {subtitleUploading ? "Uploading…" : "Upload .srt / .vtt"}
                    </span>
                  </button>
                  <input
                    ...
                    className="hidden"
                    onChange={(event) => {
                      void handleSubtitleUpload(event.target.files?.[0]);
                ...
                {movie.subtitles?.length ? (
                    ...
                    {movie.subtitles.map((subtitle) => (
                      ...
                      >
                        <span className="flex min-w-0 items-center gap-2">
                          <span className="rounded bg-surface-hover px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-muted">
                            {subtitle.language}
                          </span>
                        ...
                        <button
                          onClick={() => void handleSubtitleDelete(subtitle.id)}
                          disabled={removingSubtitleId === subtitle.id}
                          className="rounded-lg p-1.5 text-muted transition hover:bg-red-500/10 hover:text-red-500 disabled:opacity-50"
                          title="Remove subtitle"
                        >
                          {removingSubtitleId === subtitle.id ? (
                  ...
                  <p className="text-sm text-muted">
                    No subtitles attached yet. Upload an .srt or .vtt file to
                    enable captions during playback.
                  </p>
                )}
              </div>
            </div>
      ...
      {deleteOpen && (
        <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="animate-rise-in w-full max-w-md rounded-2xl border border-red-500/25 bg-surface p-6 shadow-2xl">
            <div className="flex h-11 w-11 items-center justify-center rounded-full bg-red-500/10 text-red-500">
              <AlertTriangle className="h-5 w-5" />
            </div>
            <h3 className="mt-4 text-xl font-bold">Delete this film?</h3>
            <p className="mt-2 text-sm leading-6 text-muted">
              This removes <b className="text-foreground">{movie.title}</b> from
              your library. Type the full title below to confirm.
            </p>
            <input
              ...
              autoFocus
              className="mt-5 w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/20"
            />
            <div className="mt-5 flex justify-end gap-3">
              <button
                onClick={() => setDeleteOpen(false)}
                disabled={isDeleting}
                className="rounded-xl px-4 py-2.5 text-sm font-semibold hover:bg-surface-hover"
              >
                Cancel
              </button>
              <button
                onClick={handleDelete}
                disabled={deleteConfirmation !== movie.title || isDeleting}
                className="inline-flex items-center gap-2 rounded-xl bg-red-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-40"
              >
                {isDeleting && (
                  <LoaderCircle className="h-4 w-4 animate-spin" />
                )}
