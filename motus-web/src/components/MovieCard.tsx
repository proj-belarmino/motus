...
export const MovieCard: React.FC<{
...
}> = ({ movie, onClick, favorited = false, inWatchlist = false, onChanged }) => {
  const api = useApi();
  const [imgError, setImgError] = useState(false);
  const [isFavorite, setIsFavorite] = useState(favorited);
  const [isWatchlisted, setIsWatchlisted] = useState(inWatchlist);
  const [toggling, setToggling] = useState<"favorite" | "watchlist" | null>(null);
  ...
  return (
    <div className="group relative aspect-[2/3] overflow-hidden rounded-xl border border-border bg-surface text-left shadow-sm transition duration-300 hover:-translate-y-1 hover:border-primary/50 hover:shadow-xl">
      <button onClick={onClick} className="absolute inset-0 z-0 h-full w-full cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background">
        ...
        <span className="absolute inset-0 flex items-center justify-center bg-black/0 transition group-hover:bg-black/25"><span className="flex h-11 w-11 scale-90 items-center justify-center rounded-full bg-white text-black opacity-0 shadow-xl transition group-hover:scale-100 group-hover:opacity-100"><Play className="ml-0.5 h-5 w-5 fill-current" /></span></span>
        <span className="absolute inset-x-0 bottom-0 p-3 sm:p-4"><span className="block truncate text-sm font-bold text-white sm:text-base">{movie.title}</span><span className="mt-1.5 flex items-center justify-between text-xs text-white/70"><span>{year} · {movie.metadata?.resolution || "HD"}</span>{movie.rating ? <span className="flex items-center gap-1 font-medium text-amber-300"><Star className="h-3 w-3 fill-current" /> {movie.rating}</span> : null}</span></span>
      </button>
